public class Exercise04 {
    ...
    public double calculateTotalCost(double price, int quantity) {
        int discountCase = 0;
        if (quantity >= 1 && quantity <= 15) {switchCase = 1;}
        if (quantity >= 16 && quantity <= 25) {switchCase = 2;}
        if (quantity >= 26 && quantity <= 50) {switchCase = 3;}
        if (quantity >= 51 && quantity <= 74) {switchCase = 4;}
        if (quantity >= 76) {switchCase = 5;}
        switch(discountCase){
            case 1:
